...
import { StyleSheet, Text, View, Button, Switch, Alert, TouchableOpacity } from 'react-native';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => { try { await Notifications.requestPermissionsAsync(); } catch {} })();
  }, []);
  ...
  async function startSunrise() {
    if (!cameraPermission?.granted) {
      ...
      if (!r.granted) { Alert.alert('Permiso', 'Se requiere permiso de cámara/linterna.'); return; }
    }
    ...
    try {
      ...
      if (s) await s.playAsync(); // empieza con volumen 0

      const steps = 20;
      for (let i = 1; i <= steps; i++) {
        if (cancelRef.current) break;
        const vol = i / steps;
        if (s) await s.setVolumeAsync(vol);
  ...
  async function scheduleAlarm() {
    ...
    if (preAlarmNotify && ms > 70000) {
      await Notifications.scheduleNotificationAsync({
        content: { title: 'Despertador Solar', body: 'Empieza en 1 minuto. Abre la app para linterna + tono.' },
        trigger: { seconds: Math.floor((ms - 60000) / 1000) },
      });
    }

    await Notifications.scheduleNotificationAsync({
      content: { title: 'Despertador Solar', body: '¡Amanecer ahora! Si ves esto, entra a la app.' },
      trigger: { seconds: Math.floor(ms / 1000) },
    });

    Alert.alert(
      'Programado',
      `Amanecer a las ${String(hour).padStart(2,'0')}:${String(minute).padStart(2,'0')} ${ampm}.
Deja la app visible para linterna automática (limitación iOS).`
    );
  }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <View style={styles.timeRow}>
          ...
          <View style={{width:10}} />
          <TouchableOpacity
            style={[styles.ampm, ampm==='AM'?styles.ampmActive:null]}
            onPress={()=>setAmPm('AM')}
          ><Text style={styles.ampmText}>AM</Text></TouchableOpacity>
          <TouchableOpacity
            style={[styles.ampm, ampm==='PM'?styles.ampmActive:null]}
            onPress={()=>setAmPm('PM')}
          ><Text style={styles.ampmText}>PM</Text></TouchableOpacity>
        </View>
      </View>
      ...
      <View style={styles.row}>
        ...
        <Slider
          ...
          maximumValue={30}
          step={1}
          onValueChange={(v)=>setDurationMin(Math.round(Array.isArray(v)?v[0]:v))}
          style={{width:260}}
        />
      </View>
      ...
      <Text style={styles.note}>
        iOS: para pantalla apagada usa el Atajo (linterna+sonido). La linterna desde app requiere estar en primer plano.
      </Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  ampm: { borderWidth:1, borderColor:'#3a4250', paddingVertical:6, paddingHorizontal:12, borderRadius:8 },
